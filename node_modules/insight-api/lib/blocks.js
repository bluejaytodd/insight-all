...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  return {
    ...
    bits: info.bits,
    //bits: blockObj.header.bits.toString('hex'),
    ...
    reward: this.getBlockReward(info.height) / COIN_MULTIPLE,
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var merkletree_size = br.readVarintNum();
      br.read(merkletree_size);
      ...
      for (var i = 0; i < transactions; i++) {
          // info.transactions.push(bitcore.Transaction().fromBufferReader(br));
      }
...
//         return next(err);
//       }
//       console.log(blockBuffer.length);
//       var br = new bitcore.encoding.BufferReader(blockBuffer);
//
//       // take a shortcut to get number of transactions and the blocksize.
//       // Also reads the coinbase transaction and only that.
...
var CDY_HEIGHT = 512666;
BlockController.prototype.getBlockReward = function(height) {
  var halvings;
  if (height >= CDY_HEIGHT)
    halvings = Math.floor((CDY_HEIGHT + (height - CDY_HEIGHT) / 5) / 210000);
  else
  ...
  var subsidy;
  if (height == CDY_HEIGHT) {
    subsidy = new BN(210000 * 1e8);
  } else {
    subsidy = height > CDY_HEIGHT ? new BN(10 * 1e8) : new BN(50 * 1e8);
