...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  transformed.valueOut = transaction.outputSatoshis / COIN_MULTIPLE;
  transformed.size = transaction.hex.length / 2; // in bytes
  if (!transaction.coinbase) {
    transformed.valueIn = transaction.inputSatoshis / COIN_MULTIPLE;
    transformed.fees = transaction.feeSatoshis / COIN_MULTIPLE;
  }
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.valueSat = input.satoshis / COIN_MULTIPLE;
  transformed.value = input.satoshis / COIN_MULTIPLE;
...
TxController.prototype.transformOutput = function(options, output, index) {
  var transformed = {
    value: (output.satoshis / COIN_MULTIPLE).toFixed(4),
    n: index,
    scriptPubKey: {
      hex: output.script
    }
...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  for (var i = 0; i < transaction.outputs.length; i++) {
    ...
    valueOut += output.satoshis / COIN_MULTIPLE;
    if (output.script) {
